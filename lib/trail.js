...
export const TRAIL_SCHEMA_KEY = "drTrailSchema";
export const TRAIL_SCHEMA_EXPLICIT = 2;
export const TRAIL_MAX_ENTRIES = 50;
export const TRAIL_UI_LIMIT = 12;

/**
 * 1.2.x wrote every badge prefetch into drTrail. Schema 2 is
 * explicit-lookups-only; anything else is leftover browsing history.
 * @param {unknown} schemaValue
 * @returns {boolean}
 */
export function shouldResetLegacyTrail(schemaValue) {
  return schemaValue !== TRAIL_SCHEMA_EXPLICIT;
}
...
/**
 * TSV for sheets: domain, dr, seenAt ISO, optional previous DR.
 * @param {TrailEntry[]} entries
 * @returns {string}
 */
export function formatTrailTsv(entries) {
  const header = "domain\tdr\tseenAt\tpreviousDr";
  const lines = entries.map((entry) => {
    const previousDr = entry.previous
      ? formatTrailRating(entry.previous.rating)
      : "";
    return [
      entry.domain,
      formatTrailRating(entry.rating),
      new Date(entry.seenAt).toISOString(),
      previousDr,
    ].join("\t");
  });
...
/**
 * Drop pre-1.3.0 browsing-log rows so Recent matches the explicit-lookup policy.
 * @returns {Promise<void>}
 */
export async function migrateTrailIfNeeded() {
  const result = await chrome.storage.local.get(TRAIL_SCHEMA_KEY);
  if (!shouldResetLegacyTrail(result[TRAIL_SCHEMA_KEY])) return;
  await chrome.storage.local.remove(TRAIL_STORAGE_KEY);
  await chrome.storage.local.set({ [TRAIL_SCHEMA_KEY]: TRAIL_SCHEMA_EXPLICIT });
}
